...
fn map_to_curve_simple_swu(u: &Fp2) -> G2Projective {
    ...
    G2Projective::from_raw_unchecked(x_num, y * x_den, x_den)
}
...
fn test_sgn0() {
    ...
    assert_eq!(bool::from(Fp2::new(P_M1_OVER2, Fp::zero()).sgn0()), true);
    assert_eq!(bool::from(Fp2::new(P_M1_OVER2, Fp::one()).sgn0()), true);
    assert_eq!(bool::from(Fp2::new(Fp::zero(), P_M1_OVER2).sgn0()), true);
    assert_eq!(bool::from(Fp2::new(Fp::one(), P_M1_OVER2).sgn0()), true);

    let p_p1_over2 = P_M1_OVER2 + Fp::one();
    assert_eq!(bool::from(Fp2::new(p_p1_over2, Fp::zero()).sgn0()), false);
    assert_eq!(bool::from(Fp2::new(p_p1_over2, Fp::one()).sgn0()), false);
    assert_eq!(bool::from(Fp2::new(Fp::zero(), p_p1_over2).sgn0()), false);
    assert_eq!(bool::from(Fp2::new(Fp::one(), p_p1_over2).sgn0()), true);
    assert_eq!(bool::from(Fp2::new(P_M1_OVER2, -Fp::one()).sgn0()), true);
    assert_eq!(bool::from(Fp2::new(p_p1_over2, -Fp::one()).sgn0()), false);
    assert_eq!(bool::from(Fp2::new(Fp::zero(), -Fp::one()).sgn0()), false);
    assert_eq!(bool::from(Fp2::new(P_M1_OVER2, p_p1_over2).sgn0()), true);
    assert_eq!(bool::from(Fp2::new(p_p1_over2, P_M1_OVER2).sgn0()), false);
    assert_eq!(bool::from(Fp2::new(-Fp::one(), P_M1_OVER2).sgn0()), false);
    assert_eq!(bool::from(Fp2::new(-Fp::one(), p_p1_over2).sgn0()), false);
    assert_eq!(bool::from(Fp2::new(-Fp::one(), Fp::zero()).sgn0()), false);
}
